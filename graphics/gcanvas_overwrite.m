function status = gcanvas_overwrite(gchild,cvspos)
...
gchild.Units = 'pixels';
gchild.OuterPosition(1) = cvs.GridSize(1) * (x-1) + 1; % left
gchild.OuterPosition(2) = cvs.DrawableArea(2) - cvs.GridSize(2) * (height+y-1); % bottom
gchild.OuterPosition(3) = cvs.GridSize(1) * width; % width
gchild.OuterPosition(4) = cvs.GridSize(2) * height; % height
