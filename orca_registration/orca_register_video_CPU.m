function [rt, regout] = orca_register_video_CPU(moving, template, varargin)
%orca_register_video_CPU  fast image registration using CPU and downsampling
%   RT = orca_register_video_CPU(MOVING, TEMPLATE) 
%       registers the [height*width*nFrames] video MOVING to the [height*width] TEMPLATE
%       returns [2*nFrame] RT with offsets in X-Y direction
%   [..., REGOUT] = orca_register_video_CPU(...) 
%       registers and returns registered video REGOUT
%   ... = orca_register_video_CPU(..., 'maxshift', MAXSHIFT)
%       defines maximum shift pixels in the video for all direction, by default this is 
%           1/8 of height or width
%
...
%% computation
    ...
    % ...as well as some repeatedly used variables
...
% instead of using conv2(...,'valid'), this function is way faster
